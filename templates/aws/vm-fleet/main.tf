...
resource "aws_security_group" "vm_fleet" {
  name_prefix = "vm-fleet-${random_id.vm_fleet.hex}"
  ...
  tags = {
    Name = "vm-fleet-${random_id.vm_fleet.hex}"
  }
}

resource "aws_security_group" "alb" {
  name_prefix = "vm-fleet-alb-${random_id.vm_fleet.hex}"
  vpc_id      = data.aws_vpc.default.id
  ...
  tags = {
    Name = "vm-fleet-alb-${random_id.vm_fleet.hex}"
  }
}
...
resource "aws_key_pair" "vm_fleet" {
  key_name   = "vm-fleet-${random_id.vm_fleet.hex}"
...
resource "aws_autoscaling_group" "vm_fleet" {
  ...
  tag {
    key                 = "Name"
    value               = "vm-fleet-${random_id.vm_fleet.hex}"
...
resource "aws_lb" "vm_fleet" {
  name               = "vm-${substr(random_id.vm_fleet.hex, 0, 6)}"
  internal           = false
